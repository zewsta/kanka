...
class Database:
    ...
    async def ping(self) -> None:
        ...
        This is a simple test of whether the database connection is working. It
        sends a "ping" command to the MongoDB server and logs a message if the
        connection is successful, or raises an exception if the connection fails.

        This function does not return any value, but will log a message to the
        logger if the connection is successful, or raise an exception if the
        connection fails.
        """
        try:
